...
* 3. 日常聊天场景功能由基于角色身份信息的对话能力及角色间日常故事引擎两部分构成。

* 4. 剧情聊天场景功能由基于角色身份信息的对话能力、全局剧情信息、全局剧情到当前对话剧情到推理引擎三部分组成。

* 5. 测试了transformers、llama-cpp-python、vLLM三种推理框架在这两个对话场景下的使用，在transformers场景下使用lora switch的方法给出低显存占用条件下的功能实现，在llama-cpp-python、vLLM场景下通过模型分别合并部署和加速给出高推理速度需求下的功能实现，可根据需求灵活选取。
