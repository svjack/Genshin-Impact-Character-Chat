...
qw_model.load_adapter("svjack/DPO_Genshin_Impact_Qwen_1_5_Plot_Engine_Step_Json_Short_lora_small",
                     adapter_name = "engine"
                     )
qw_model = qw_model.eval()
qw_model.set_adapter("chat")
qw_model.set_adapter("engine")
...
def qwen_hf_predict(messages, qw_model = qw_model,
    ...
    qw_model.set_adapter(adapter_name)
...
def model_chat(query: Optional[str], history: Optional[History],
    ...
    req_iter = qwen_hf_predict_stream(messages, temperature = 0.5)
...
with gr.Blocks() as demo:
    gr.Markdown("""<center><font size=8>😁 Genshin Impact Qwen-1.5-7B-Chat Plot Roleplay Turned Transformer Bot 🔥</center>""")
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            chatbot = gr.Chatbot(
                label='svjack/Genshin_Impact_Qwen_1_5_Plot_Chat_roleplay_chat_lora_small/DPO_Genshin_Impact_Qwen_1_5_Plot_Engine_Step_Json_Short_lora_small',
                height = 768
                )
